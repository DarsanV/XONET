...
type ExperienceEditorProps = {
  ...
  editing: boolean;
  onAdd: (entry: Omit<ExperienceEntry, "id">) => void;
...
export function ExperienceEditor({
  ...
  const [form, setForm] = useState<Omit<ExperienceEntry, "id">>(emptyEntry());
      ...
      <div className="space-y-4">
        {experience.map((entry, index) => (
          <div
            key={entry.id}
            className={cn(
              "rounded-md border border-border bg-background/50 p-4 transition-all duration-200",
              editing && "hover:border-foreground/20",
            )}
        ...
        {editing && (
          <Button
            type="button"
            variant="secondary"
            className="h-9 w-full gap-2 rounded-md animate-in fade-in duration-200"
            onClick={openAdd}
          >
