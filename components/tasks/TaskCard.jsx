...
export function TaskCard({ task, showApply = true, showStatus = false }) {
    ...
    function handleApply() {
        ...
        const result = applyToTask(task.id, CURRENT_FREELANCER_ID, coverLetter.trim(), proposedRate.trim() || undefined);
        if (result) {
      ...
      <Card className="group flex flex-col border-border bg-card shadow-[0_1px_0_0_rgba(255,255,255,0.03)_inset] transition-colors hover:border-foreground/30">
        <CardContent className="flex h-full flex-col p-5">
          ...
          <div className="mt-auto flex items-center justify-between border-t border-border pt-4">
            ...
            {canApply && (<Button size="sm" className="h-9 rounded-md px-4" onClick={() => setOpen(true)}>
                Apply
              </Button>)}
            {applied && (<Badge variant="secondary" className="rounded-sm text-[10px]">
                Applied
              </Badge>)}
            {showStatus && task.status !== "Open" && !showApply && (<span className="text-xs text-muted-foreground">{task.status}</span>)}
          </div>
        </CardContent>
      </Card>
        ...
        <DialogContent className="border-border bg-card sm:max-w-md">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Textarea value={coverLetter} onChange={(e) => setCoverLetter(e.target.value)} placeholder="Briefly explain why you're a strong fit…" className="min-h-[120px] resize-none border-border bg-background"/>
            </div>
            <div className="space-y-2">
              <Label className="text-xs uppercase tracking-wider text-muted-foreground">
                Proposed rate (optional)
              </Label>
              <Input value={proposedRate} onChange={(e) => setProposedRate(e.target.value)} placeholder="e.g. $3,500 fixed" className="h-10 border-border bg-background text-sm"/>
            </div>
          </div>
          <DialogFooter>
