...
import { earningsTrend, projectStatus } from "@/lib/dummy-data";
...
export function DashboardView() {
    ...
    return (<div className="space-y-8">
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((s) => (<Card key={s.label} className="border-border bg-card shadow-[0_1px_0_0_rgba(255,255,255,0.03)_inset]">
      ...
      <section>
        <div className="mb-4 flex items-end justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Live from Explore — updates when new tasks are posted.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {featured.map((task) => (<TaskCard key={task.id} task={task} showApply/>))}
        </div>
      </section>
    </div>);
}
