...
export function ExploreView() {
  ...
  const filtered = useMemo(() => {
    ...
    if (budget === "1k") list = list.filter((t) => parseBudget(t.budget) < 3000);
    if (budget === "3k") {
      list = list.filter((t) => {
    ...
    if (budget === "6k") list = list.filter((t) => parseBudget(t.budget) >= 6000);
    return list;
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        description="Browse open opportunities ranked by relevance — including tasks posted in real time."
      />
      ...
      {filtered.length === 0 ? (
        <Card className="border-border bg-card py-20 text-center">
