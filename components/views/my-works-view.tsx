...
function WorkCard({ work }: { work: WorkItem }) {
  ...
  return (
    <Card
      className={cn(
        "border-border bg-card p-5 shadow-[0_1px_0_0_rgba(255,255,255,0.03)_inset]",
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <Badge
          className={`rounded-sm px-2 py-0.5 text-[10px] font-medium ${statusStyle(work.status)}`}
        >
          {work.status}
        </Badge>
      </div>
...
function WorksGrid({ items }: { items: WorkItem[] }) {
  if (items.length === 0) {
    return (
