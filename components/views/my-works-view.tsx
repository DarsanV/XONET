      ...
      <Table>
        <TableHeader>
          <TableRow className="border-border hover:bg-transparent">
            ...
            <TableHead className="w-[200px] text-[11px] uppercase tracking-wider text-muted-foreground">
              Progress
            </TableHead>
            <TableHead className="text-[11px] uppercase tracking-wider text-muted-foreground">
              Deadline
        ...
        <TableBody>
          {items.map((w) => (
            <TableRow key={w.id} className="border-border">
              ...
              <TableCell>
                <Badge
                  className={`rounded-sm px-2 py-0.5 text-[10px] font-medium ${statusStyle(w.status)}`}
                >
                  {w.status}
                </Badge>
              </TableCell>
    ...
    <div className="space-y-8">
      ...
      <Tabs defaultValue="active" className="space-y-4">
        ...
        <TabsContent value="active">
          <WorksTable items={active} />
        </TabsContent>
        <TabsContent value="completed">
          <WorksTable items={completed} />
        </TabsContent>
        <TabsContent value="all">
          <WorksTable items={myWorks} />
        </TabsContent>
      </Tabs>
    </div>
  );
