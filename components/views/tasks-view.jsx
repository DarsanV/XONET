...
export function TasksView() {
    ...
    const { tasks, applications, assignedTasks, updateTask, deleteTask, getFreelancer } = useTaskStore();
    ...
    return (<div className="space-y-8">
      ...
      <section className="space-y-4 animate-in fade-in duration-300">
        ...
        {assignedTasks.length === 0 ? (<Card className="border-border bg-card py-12 text-center text-sm text-muted-foreground">
            ...
            {assignedTasks.map((task) => {
                ...
                if (!freelancer)
                    return null;
                return (<AssignedTaskCard key={task.id} task={task} freelancer={freelancer} onView={() => setDetailTask(task)} onOpenChat={() => setChatTask(task)}/>);
            })}
          </div>)}
      </section>
      ...
      <Tabs value={mainTab} onValueChange={setMainTab} className="space-y-4">
        ...
        <TabsContent value="posted" className="space-y-4">
          <PostedTasksTable tasks={postedTasks} onView={setDetailTask} onEdit={setEditTask} onDelete={setDeleteId} onOpenChat={setChatTask} onStatusChange={handleStatusChange} onPaymentChange={handlePaymentChange}/>
        </TabsContent>
      ...
      <TaskDetailSheet task={detailTask} open={!!detailTask} onOpenChange={(o) => !o && setDetailTask(null)} onOpenChat={(t) => {
            setDetailTask(null);
            setChatTask(t);
        }}/>

      <TaskChatPanel task={chatTask} open={!!chatTask} onOpenChange={(o) => !o && setChatTask(null)}/>
      ...
      <Dialog open={!!editTask} onOpenChange={(o) => !o && setEditTask(null)}>
        <DialogContent className="max-h-[90vh] overflow-y-auto border-border bg-card sm:max-w-2xl">
          ...
          {editTask && (<TaskForm defaultValues={editTask} submitLabel="Save changes" onCancel={() => setEditTask(null)} onSubmit={(data) => {
...
function PostedTasksTable({ tasks, onView, onEdit, onDelete, onOpenChat, onStatusChange, onPaymentChange, }) {
    if (tasks.length === 0) {
    ...
    return (<Card className="overflow-hidden border-border bg-card">
      <Table>
        ...
        <TableBody>
          {tasks.map((task) => (<TableRow key={task.id} className="border-border">
              <TableCell>
              ...
              <TableCell>
                <Select value={task.paymentStatus} onValueChange={(v) => onPaymentChange(task.id, v)}>
              ...
              <TableCell>
                <div className="flex justify-end gap-1">
                  {task.assignedFreelancerId && (<Button size="icon" variant="ghost" className="h-8 w-8" title="Open chat" onClick={() => onOpenChat(task)}>
                      <MessageSquare className="h-3.5 w-3.5"/>
                    </Button>)}
                  <Button size="icon" variant="ghost" className="h-8 w-8" title="View details" onClick={() => onView(task)}>
