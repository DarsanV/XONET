...
export function FreelancersView() {
    ...
    const filtered = useMemo(() => {
        ...
        if (!q)
            return freelancers;
        return freelancers.filter((f) => f.name.toLowerCase().includes(q) ||
            f.headline.toLowerCase().includes(q) ||
            f.skills.some((s) => s.toLowerCase().includes(q)));
    }, [freelancers, query]);
    return (<div className="space-y-8">
      <PageHeader eyebrow="Freelancers" title="Talent directory" description="Discover specialists ranked by fit score, availability, and skill overlap with your open tasks."/>

      <Card className="border-border bg-card">
        <CardContent className="p-4">
          <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search by name, skill, or headline…" className="h-10 border-border bg-background text-sm"/>
        </CardContent>
      </Card>

      <section className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {filtered.map((f) => (<Card key={f.id} className="border-border bg-card shadow-[0_1px_0_0_rgba(255,255,255,0.03)_inset] transition-colors hover:border-foreground/30">
